...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        **********************************************************************
        ...
        for m in legal_moves:
            v = self.__min_value(game.forecast_move(m), depth - 1, alpha, beta)
            alpha = max(alpha, v)
            if v > best_score:
                best_score = v
                best_move = m
    ...
    def __min_value(self, game, depth, alpha, beta):
        ...
        v = float('inf')
        for m in game.get_legal_moves():
            v = min(v, self.__max_value(game.forecast_move(m), depth - 1, alpha, beta))
            if v <= alpha:
                return v
            beta = min(beta, v)
        return v
    ...
    def __max_value(self, game, depth, alpha, beta):
        ...
        v = float('-inf')
        for m in game.get_legal_moves():
            v = max(v, self.__min_value(game.forecast_move(m), depth - 1, alpha, beta))
            if v >= beta:
                return v
            alpha = max(alpha, v)
        return v
